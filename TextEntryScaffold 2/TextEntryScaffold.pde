...
final float smallerCircleSize = sizeOfInputArea / 3; // Adjust the size of the smaller circle

PImage watch;
PImage finger;
float knobRadius = 100; // Radius of the knob
float knobAngle = 0; // Initial angle of the knob
float knobX, knobY; // X and Y positions of the knob
boolean dragging = false; // Flag to track dragging
...
void draw()
{
  ...
  if (startTime!=0)
  {
    ...
    fill(200);
    ellipse(knobX, knobY, knobRadius*1.2, knobRadius*1.2);
    
      //Draw a smaller green circle inside the square
    fill(0, 200, 0); // Darker green color for the circle
    ellipse(width/2, height/2, smallerCircleSize, smallerCircleSize);
    
    
    float knobPosX = knobX + cos(radians(knobAngle)) * knobRadius/2;
    float knobPosY = knobY + sin(radians(knobAngle)) * knobRadius/2;
    
      
    // Draw the knob handle
    fill(100);
    ellipse(knobPosX, knobPosY, 20, 20);
    
    // Display the currentLetter based on the angle
...
boolean didMouseClick2(float x, float y, float w, float h) {
  return (mouseX > x && mouseX < x + w && mouseY > y && mouseY < y + h);
}
...
void mousePressed()
{
  if (didMouseClick2(width / 2 - smallerCircleSize / 2, height / 2 - smallerCircleSize / 2, smallerCircleSize, smallerCircleSize)) 
  {
  ...
  float distance = dist(mouseX, mouseY, knobX, knobY);
...
char getCurrentLetter() {
  // Map the angle to the range of characters ('a' to 'z' and '_')
  int index = floor(map(knobAngle, 0, 360, 0, 28));
  char[] letters = {'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', '_', '`'};
  return letters[(index + letters.length) % letters.length]; // Ensure cycling within bounds
}
