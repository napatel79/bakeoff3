...
PImage watch;
PImage finger;
PVector[] numberedButtons = new PVector[12]; // 11 buttons from 0 to 9 and the apostrophe (')
...
void draw()
{
  ...
  if (startTime!=0)
  {
    ...
    rect(width/2-sizeOfInputArea/2, height/2-sizeOfInputArea/2, sizeOfInputArea, sizeOfInputArea/4); //draw right green button
    textAlign(CENTER);
    fill(0);
    text("" + currentLetter, width/2, height/2-sizeOfInputArea/2.8); //draw current letter
    fill(0);
    text(": " + lettersEnteredTotal, width/2+sizeOfInputArea/3, height/2-sizeOfInputArea/2.8);
 ...
 //width/2-sizeOfInputArea/2, height/2-sizeOfInputArea/2+sizeOfInputArea/2
  ...
  float startY =height/2-sizeOfInputArea/2+sizeOfInputArea/3;
  float spacing = 30;
  int buttonsPerRow = 4;
  
  for (int i = 0; i < 12; i++) {
    int row = i / buttonsPerRow;
    float x = startX + (i % buttonsPerRow) * spacing;
    float y = startY + row * (buttonSize + 10);
    ...
    if (i < 10) {
      ...
      text("'", x + buttonSize / 2, y + buttonSize / 2);
    } else {
      text("_", x + buttonSize / 2, y + buttonSize / 3);
    }
  }
  // Display the running total
  //fill(0);
  //textAlign(CENTER, CENTER);
  //text("Total: " + lettersEnteredTotal, width / 2, height - 50);
  fill(0);
  textAlign(LEFT, CENTER);
  text("Legend: ", 40, height - 100);

  int charactersPerLine = 6;
  for (int i = 0; i < 26; i++) {
    char letter = (char)('a' + i);
    int row = i / charactersPerLine;
    int col = i % charactersPerLine;
    text(letter + " = " + i + "   ", 120 + col * 80, height - 100 + row * 20);
  }
  }
  ...
  for (int i = 0; i < 12; i++) {
    ...
    if (mouseX > x && mouseX < x + buttonSize && mouseY > y && mouseY < y + buttonSize) {
      if (i < 10) {
        ...
        else if (lettersEnteredTotal + i <= 27) {
          lettersEnteredTotal += i;
          currentLetter = (char)('a' + (lettersEnteredTotal % 26)); // Wrap around 'a' to 'z'
        } else {
          lettersEnteredTotal = (lettersEnteredTotal + i) % 28; // Loop back to 0 if total exceeds 27
          currentLetter = (char)('a' + (lettersEnteredTotal % 26)); // Wrap around 'a' to 'z'
        }
      } else if (i == 10) {
        currentLetter = '`';
        println("Apostrophe clicked");
      } else {
        currentLetter = '_';
        println("Underscore clicked");
      }
    }
  }
